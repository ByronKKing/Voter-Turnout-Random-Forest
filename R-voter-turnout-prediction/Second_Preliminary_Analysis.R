#################prepare second dataset for analysis
...
#modify dataset--new dataset w/ vars. below 23,000 missing values (i.e. include education)
analysis1<-analysis1[colSums(is.na(analysis1)) < 23000]
#dataset w/ complete cases of these variables for random forest
...
analysis1$genhist<-rowSums( analysis1[,30:36] )
...
RF2 <- randomForest(as.factor(turnout) ~ age + party + ethnicity + income + cd + dma + education +
...
Prediction <- predict(RF2, analysis1Test)
RFPRED2 <- data.frame(optimus_id = analysis1Test$optimus_id, 
